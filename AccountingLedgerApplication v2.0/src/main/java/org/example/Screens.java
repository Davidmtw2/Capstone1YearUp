...
public class Screens {
    ...
    public Screens(TransactionService transactionService) {
        this.transactionService = transactionService;
        this.reportGenerator = new ReportGenerator(transactionService);
    ...
    public void customSearch() {
        ...
        List<Transaction> results = transactionService.customSearch(startDate, endDate, description, vendor, amount);
    ...
    public void displayDeposits() {
        List<Transaction> deposits = transactionService.filterDeposits();
        if (deposits.isEmpty()) {
            simulateTyping("No deposits found.\n", 1);
        } else {
            for (Transaction deposit : deposits) {
                simulateTyping(deposit.toString() + "\n", 1);
            }
        }
    }

    public void displayPayments() {
        List<Transaction> payments = transactionService.filterPayments();
        if (payments.isEmpty()) {
            simulateTyping("No payments found.\n", 1);
        } else {
            for (Transaction payment : payments) {
                simulateTyping(payment.toString() + "\n", 1);
            }
        }
    }
